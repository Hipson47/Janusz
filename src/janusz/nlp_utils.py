        ...
        try:
            ...
            for chunk in doc.noun_chunks:
                if len(chunk.text.strip()) > 3 and chunk.text.lower().strip() not in STOPWORDS:
                    keywords.append(Keyword(text=chunk.text.strip(), confidence_level="high"))

            # Extract named entities
            for ent in doc.ents:
                if ent.label_ in ["ORG", "PRODUCT", "GPE", "PERSON", "WORK_OF_ART"]:
                    keywords.append(Keyword(text=ent.text.strip(), confidence_level="high"))
        ...
        try:
            ...
            for word, tag in tagged:
                if (
                    ...
                    keywords.append(Keyword(text=word, confidence_level="medium"))
...
def extract_keywords_fallback(text: str) -> list[Keyword]:
    ...
    for word in capitalized:
        if word.lower() not in STOPWORDS:
            keywords.append(Keyword(text=word, confidence_level="low"))
    ...
    for term in technical:
        keywords.append(Keyword(text=term, confidence_level="medium"))
