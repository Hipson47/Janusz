...
class PromptTester:
    ...
    async def test_prompt(
        ...
        tasks = [test_single_case(i, test_case) for i, test_case in enumerate(test_cases)]
    ...
    async def benchmark_prompts(
        ...
        for prompt_id, prompt_text in prompts.items():
            ...
            logger.info(
                f"Completed benchmarking {prompt_id}: avg_score={benchmark_result.average_score:.3f}"
            )
    ...
    async def compare_prompts(
        ...
        if baseline_prompt_id and baseline_prompt_id in [r.prompt_id for r in benchmark_results]:
            baseline_result = next(
                r for r in benchmark_results if r.prompt_id == baseline_prompt_id
            )
            ...
            for result in benchmark_results:
                if result.prompt_id != baseline_prompt_id:
                    ...
                    improvement_pct = (
                        (improvement / baseline_result.average_score) * 100
                        if baseline_result.average_score > 0
                        else 0
                    )
        ...
        return {
            ...
            "performance_spread": (
                sorted_results[0].average_score - sorted_results[-1].average_score
        ...
        try:
            ...
            if response:
                ...
                quality_score = self._evaluate_response_quality(
                    response, expected_output, test_input
                )
    ...
    def _calculate_detailed_metrics(
        ...
        avg_words_per_sentence = metrics["response_length_words"] / max(
            1, metrics["response_length_sentences"]
        )
        ...
        metrics["complexity_score"] = min(
            1.0, complex_words / max(1, metrics["response_length_words"])
        )
